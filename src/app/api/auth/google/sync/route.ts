 ...
 * Google OAuth 完成後，同步用戶到 players 表
 */
export async function POST(request: NextRequest) {
  try {
    const { googleId, email, displayName, avatarUrl, referralCode } = await request.json();

    if (!googleId) {
      return NextResponse.json({ error: "缺少 Google ID" }, { status: 400 });
    }
    ...
    if (existingPlayer) {
      ...
      if (insertError) {
        console.error("[Google sync] Insert error:", insertError);
      ...
      if (inviterId) {
        try {
          ...
          console.error("[Google sync] Referral reward error:", refErr);
        }
      }
    }
